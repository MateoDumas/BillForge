...
export function RevenueChart({ invoices }: RevenueChartProps) {
  ...
  if (chartData.length === 0) {
    return (
        <div className="card col-span-full flex items-center justify-center p-8 text-muted">
            No hay datos suficientes para mostrar el historial de ingresos.
        </div>
    );
  }
  ...
  return (
    <div className="card col-span-full">
      <h3 className="card-title mb-4">Historial de Ingresos</h3>
      <div style={{ height: '300px', width: '100%' }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData}>
            ...
            />
            <Tooltip 
              formatter={(value: any) => [`$${Number(value).toFixed(2)}`, 'Ingresos'] as [string, string]}
